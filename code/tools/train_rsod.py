...
def parse_args():
    parser = argparse.ArgumentParser(description='Train a detector on RSOD dataset')
    ...
    parser.add_argument('--data-root', type=str, default='/mnt/cs_dsns_public/share/group_lvxiaodan/RSOD',
                       help='RSOD dataset root directory (containing Annotations/ and JPEGImages/)')
    parser.add_argument('--num-classes', type=int, default=4,
                       help='Number of classes in RSOD dataset (default: 4)')
...
def setup_rsod_config(cfg, args):
    """Setup RSOD dataset specific configurations"""
    
    train_ann = osp.join(args.data_root, 'Annotations', 'RSOD_trainval.json')
    val_ann   = osp.join(args.data_root, 'Annotations', 'RSOD_test.json')
    img_prefix = osp.join(args.data_root, 'JPEGImages')
...
def main():
    ...
    cfg = setup_rsod_config(cfg, args)
    ...
    if args.work_dir is not None:
        ...
        cfg.work_dir = osp.join('./work_dirs', 'rsod', config_name)
    ...
    if rank == 0:
        ...
        logger.info(f'RSOD Dataset Root: {args.data_root}')
