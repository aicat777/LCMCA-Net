...
def parse_args():
    parser = argparse.ArgumentParser(description='Train a detector on NWPU-VHR10 dataset')
    ...
    parser.add_argument('--data-root', type=str, default='/mnt/cs_dsns_public/share/group_lvxiaodan/NWPU_VHR10',
                       help='NWPU-VHR10 dataset root directory (containing images/ and annotations/)')
    parser.add_argument('--num-classes', type=int, default=10,
                       help='Number of classes in NWPU-VHR10 dataset (default: 10)')
...
def setup_nwpu_config(cfg, args):
    """Setup NWPU-VHR10 dataset specific configurations"""
    train_ann = osp.join(args.data_root, 'instances_train2017.json')
    val_ann   = osp.join(args.data_root, 'instances_val2017.json')
    img_prefix = osp.join(args.data_root, 'image')   
    ...
    if cfg.model.get('rpn_head'):
        if 'num_classes' in cfg.model.rpn_head:
            cfg.model.rpn_head.num_classes = args.num_classes
    if cfg.model.get('mask_head'):
        if 'num_classes' in cfg.model.mask_head:
            cfg.model.mask_head.num_classes = args.num_classes
...
def main():
    ...
    cfg = setup_nwpu_config(cfg, args)
    ...
    if args.work_dir is not None:
        ...
        cfg.work_dir = osp.join('./work_dirs', 'nwpu', config_name)
    ...
    if rank == 0:
        ...
        logger.info(f'NWPU-VHR10 Dataset Root: {args.data_root}')
