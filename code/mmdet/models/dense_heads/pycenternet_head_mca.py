...
# constant_init 应该在 mmcv.cnn 里导入
...
from mmcv.ops import DeformConv2d, ModulatedDeformConv2dPack, ModulatedDeformConv2d
...
import torch
import torch.nn as nn
# 假设 ModulatedDeformConv2d, constant_init, normal_init 等已在你的代码库中导入

class OffsetPredictorDW(nn.Module):
    """
    Depthwise-separable version of OffsetPredictor.
    Replaces the three standard dilated convolutions with
    depthwise-separable convolutions (depthwise + pointwise).
    """
    def __init__(self, channels):
        super(OffsetPredictorDW, self).__init__()
        
        # Depthwise layers (one per dilation), each followed by a shared or separate pointwise.
        # We'll use separate pointwise for each branch to keep them independent.
        self.dw_conv_r3 = nn.Conv2d(channels, channels, kernel_size=3, 
                                    padding=1, dilation=1, groups=channels, bias=False)
        self.pw_conv_r3 = nn.Conv2d(channels, channels, kernel_size=1)

        self.dw_conv_r5 = nn.Conv2d(channels, channels, kernel_size=3, 
                                    padding=2, dilation=2, groups=channels, bias=False)
        self.pw_conv_r5 = nn.Conv2d(channels, channels, kernel_size=1)

        self.dw_conv_r7 = nn.Conv2d(channels, channels, kernel_size=3, 
                                    padding=3, dilation=3, groups=channels, bias=False)
        self.pw_conv_r7 = nn.Conv2d(channels, channels, kernel_size=1)

        # BatchNorm and activation
        self.bn = nn.BatchNorm2d(channels * 3)
        self.relu = nn.ReLU(inplace=True)
        
        # Final 1x1 to produce 3-channel output (offset_x, offset_y, mask)
        self.out_conv = nn.Conv2d(channels * 3, 3, kernel_size=1)

    def forward(self, x):
        # r=1 branch
        f3 = self.dw_conv_r3(x)
        f3 = self.pw_conv_r3(f3)

        # r=2 branch
        f5 = self.dw_conv_r5(x)
        f5 = self.pw_conv_r5(f5)

        # r=3 branch
        f7 = self.dw_conv_r7(x)
        f7 = self.pw_conv_r7(f7)

        # Concatenate
        feat_cat = torch.cat([f3, f5, f7], dim=1)  # 3*channels
        feat_cat = self.relu(self.bn(feat_cat))
        out = self.out_conv(feat_cat)
        return out
...
class DCAModule(nn.Module):
    def __init__(self, channels):
        ...
        # 修改点：使用自定义的多感受野 OffsetPredictor 替换原有的单一卷积 Sequential
        self.offset_tl = OffsetPredictorDW(channels)
        self.offset_br = OffsetPredictorDW(channels)
        
        self.dcn_tl = ModulatedDeformConv2d(
            channels, channels, kernel_size=1, stride=1, padding=0, bias=False
        )
        self.dcn_br = ModulatedDeformConv2d(
            channels, channels, kernel_size=1, stride=1, padding=0, bias=False
        )
        
        self.bn_tl = nn.BatchNorm2d(channels)
        self.bn_br = nn.BatchNorm2d(channels)
        
        self.gamma_tl = nn.Parameter(torch.full((1, channels, 1, 1), 1e-4))
        self.gamma_br = nn.Parameter(torch.full((1, channels, 1, 1), 1e-4))

    def init_weights(self):
        # 注意修改点：由于封装了 OffsetPredictor，初始化末端卷积时需要指定到 .out_conv
        constant_init(self.offset_tl.out_conv, 0)
        constant_init(self.offset_br.out_conv, 0)
        normal_init(self.dcn_tl, std=0.01)
        normal_init(self.dcn_br, std=0.01)

    def forward(self, query_ct, value_tl, value_br):
        
        # 处理左上角 (TL)
        out_offset_tl = self.offset_tl(query_ct)
        off_tl = out_offset_tl[:, :2, :, :]
        mask_tl = torch.sigmoid(out_offset_tl[:, 2:, :, :])
        
        feat_fetched_tl = self.dcn_tl(value_tl, off_tl, mask_tl)
        feat_fetched_tl = self.bn_tl(feat_fetched_tl)
        
        # 处理右下角 (BR)
        out_offset_br = self.offset_br(query_ct)
        off_br = out_offset_br[:, :2, :, :]
        mask_br = torch.sigmoid(out_offset_br[:, 2:, :, :])
        
        feat_fetched_br = self.dcn_br(value_br, off_br, mask_br)
        feat_fetched_br = self.bn_br(feat_fetched_br)
        
        # 门控残差融合
        out = query_ct + self.gamma_tl * feat_fetched_tl + self.gamma_br * feat_fetched_br
        
        return out

@HEADS.register_module()
class PyCenterNetHeadV7(AnchorFreeHead):
    def __init__(self,
                 num_classes,
                 in_channels,
    ...
    def _init_layers(self):
        ...
        self.dca = DCAModule(self.feat_channels)
    ...
    def forward(self, feats):
        levels = list(range(len(feats)))
        return multi_apply(self.forward_single, feats, levels)

    def forward_single(self, x,lvl):
        ...
        hem_ct_feat = self.dca(query_ct=hem_ct_feat, value_tl=hem_tl_feat, value_br=hem_br_feat)
    ...
    def loss(self,
        ...
        losses_cls, losses_pts_init, losses_pts_refine, losses_heatmap, losses_offset= multi_apply(
            ...
            self.point_strides, # <--- 传给 stride 的参数在这里
